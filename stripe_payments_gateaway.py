...
def create_payment(client_id: str, payment_method_id: str, product_id: str, amount: int, currency: str):
    ...
    try:
        ...
        print(f"Error en la tarjeta: {error.user_message}")
    except stripe.error.StripeError as error:
        print(f"Error en Stripe: {error.user_message}")
...
def add_payment_method_to_user(client_id, payment_method_id):
    ...
    print(f"Método de pago asociado al usuario.")


#Recuperar los diferentes productos
...
# Pruebas
client_id = create_user("Cristian Trapiello", "ct@gmail.com")
